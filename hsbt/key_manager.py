...
class KeyManager:
    ...
    def ssh_keygen(self, overwrite_if_exists: bool = False, exists_ok=False, key_length:int = 2048):
        ...
        if self.validate_if_keys_exists_and_valid() and not overwrite_if_exists and not exists_ok:
            raise FileExistsError(f"Can not generate key at {self.private_key_path}, because file allready exists. Use the `overwrite_if_exist` param if you want to overwrite it. Or use the `exists_ok` param if the existing file is ok")
        elif self.validate_if_keys_exists_and_valid() and not overwrite_if_exists and exists_ok:
            return 
        run_command(
    ...
    def gen_rfc4716_format_copy(
        self, target_path:Union[Path, str]=None,overwrite_if_exist: bool = False, exists_ok=False
    ) -> Path:
        if isinstance(target_path, str):
            target_path: Path = Path(target_path)
        elif target_path is None:
            target_path = Path(PurePath(self.public_key_path.parent, self.public_key_path.stem + ".rfc.pub"))
        if target_path.is_file() and not overwrite_if_exist and not exists_ok:
            raise FileExistsError(f"Can not convert {self.public_key_file} into RFC 4716 format, because file at target path `{target_path}` allready exists. Use the `overwrite_if_exist` param if you want to overwrite it. Or use the `exists_ok` param if the existing file is ok")
    ...
    def get_public_key(self,  in_rfc4716_format: bool = False) -> bytes:
        key_file_path = self.public_key_rfc_path if in_rfc4716_format else self.public_key_path
